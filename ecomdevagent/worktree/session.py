...
from ecomdevagent.worktree.models import WorktreeSession
...
def _session_path(ecomdevagent_dir: Path) -> Path:
    return ecomdevagent_dir / SESSION_FILENAME


def save_worktree_session(
    ecomdevagent_dir: Path,
    session: WorktreeSession | None,
) -> None:
    path = _session_path(ecomdevagent_dir)
...
def load_worktree_session(ecomdevagent_dir: Path) -> WorktreeSession | None:
    path = _session_path(ecomdevagent_dir)
    if not path.exists():
        return None
    try:
