...
from ecomdevagent.worktree.changes import (
    CleanupResult,
    Changes,
    count_worktree_changes,
    has_worktree_changes,
)
from ecomdevagent.worktree.models import Worktree, WorktreeSession
from ecomdevagent.worktree.session import load_worktree_session, save_worktree_session
from ecomdevagent.worktree.setup import perform_post_creation_setup
from ecomdevagent.worktree.slug import flatten_slug, validate_slug
...
class WorktreeManager:
    def __init__(
        ...
        self.worktree_dir = worktree_dir or str(
            Path(repo_root) / ".ecomdevagent" / "worktrees"
        )
        self._ecomdevagent_dir = Path(repo_root) / ".ecomdevagent"
    ...
    async def enter(self, name: str) -> WorktreeSession:
        ...
        save_worktree_session(self._ecomdevagent_dir, session)
        return session
    ...
    async def exit(
        ...
        save_worktree_session(self._ecomdevagent_dir, None)
    ...
    def restore_session(self) -> WorktreeSession | None:
        session = load_worktree_session(self._ecomdevagent_dir)
        if session is None:
            return None
