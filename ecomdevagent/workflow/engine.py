...
from ecomdevagent.workflow.context import BudgetExhaustedError, WorkflowContext
from ecomdevagent.workflow.journal import JOURNALS_DIR, Journal
from ecomdevagent.workflow.models import BudgetInfo, WorkflowDef, WorkflowState

log = logging.getLogger(__name__)

WORKFLOWS_DIR = ".ecomdevagent/workflows"
...
class WorkflowEngine:
    ...
    负责：
    - 扫描 .ecomdevagent/workflows/ 目录发现 workflow
    - 加载 Python 模块并提取入口函数
    ...
    def list_workflows(self) -> list[WorkflowDef]:
        """扫描 .ecomdevagent/workflows/ 目录，返回所有可用 workflow 的元信息。"""
    ...
    def _load_module(self, workflow_name: str) -> Any:
        ...
        # 使用唯一的模块名避免冲突
        mod_name = f"ecomdevagent_workflow_{workflow_name}"
