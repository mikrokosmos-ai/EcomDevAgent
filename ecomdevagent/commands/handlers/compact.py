...
from ecomdevagent.commands.registry import Command, CommandContext, CommandType
...
async def handle_compact(ctx: CommandContext) -> None:
    ...
    from ecomdevagent.agent import CompactNotification, ErrorEvent
    ...
    if isinstance(result, CompactNotification):
        ...
        if ctx.session is not None and result.boundary is not None:
            from ecomdevagent.memory.session import make_compact_boundary
