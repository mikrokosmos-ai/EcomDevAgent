...
from ecomdevagent.client import LLMClient
from ecomdevagent.context import (
    CompactBoundary,
...
from ecomdevagent.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from ecomdevagent.conversation import ThinkingBlock as ConvThinkingBlock
from ecomdevagent.memory.auto_memory import MemoryManager
from ecomdevagent.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from ecomdevagent.hooks import HookContext, HookEngine, ToolRejectedError
from ecomdevagent.hooks.engine import HookNotification
from ecomdevagent.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from ecomdevagent.tools import ToolRegistry
from ecomdevagent.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".ecomdevagent" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".ecomdevagent" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from ecomdevagent.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from ecomdevagent.context.manager import (
