...
from ecomdevagent.agent import (
    Agent,
    CompactNotification,
    ErrorEvent,
    HookEvent,
...
from ecomdevagent.client import (
    AuthenticationError,
    LLMClient,
    LLMError,
    create_client,
...
from ecomdevagent.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from ecomdevagent.commands.completion import CompletionPopup
from ecomdevagent.commands.handlers import register_all_commands
from ecomdevagent.config import MCPServerConfig, ProviderConfig
from ecomdevagent.hooks import HookContext, HookEngine, load_hooks
from ecomdevagent.conversation import ConversationManager, Message
from ecomdevagent.mcp import MCPManager
from ecomdevagent.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from ecomdevagent.agents.notification import inject_task_notifications
from ecomdevagent.commands.handlers.tasks import create_tasks_command
from ecomdevagent.skills.executor import SkillExecutor
from ecomdevagent.skills.loader import SkillLoader
from ecomdevagent.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from ecomdevagent.cache import FileCache
from ecomdevagent.tools import ToolRegistry, create_default_registry
from ecomdevagent.tools.agent_tool import AgentTool
from ecomdevagent.tools.ask_user import AskUserEvent, AskUserTool
from ecomdevagent.tools.impl.tool_search import ToolSearchTool
from ecomdevagent.tools.load_skill import LoadSkill
from ecomdevagent.worktree.cleanup import start_stale_cleanup_task
from ecomdevagent.worktree.manager import WorktreeManager
from ecomdevagent.commands.handlers.worktree import create_worktree_command
from ecomdevagent.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".ecomdevagent", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".ecomdevagent" / "history"
...
class EcomDevAgentApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_ECOMDEVAGENT_THEME)
        self.theme = "ecomdevagent"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        from ecomdevagent.filehistory import FileHistory
        ...
        from ecomdevagent.tools.exit_plan_mode import ExitPlanModeTool
        ...
        from ecomdevagent.tools.enter_worktree import EnterWorktreeTool
        from ecomdevagent.tools.exit_worktree import ExitWorktreeTool
        ...
        from ecomdevagent.commands.handlers.trace import create_trace_command
        ...
        from ecomdevagent.tools.synthetic_output import SyntheticOutputTool
    ...
    def _init_workflow_engine(self, provider: ProviderConfig, work_dir: str) -> None:
        ...
        from ecomdevagent.workflow.tool import ListWorkflowsTool, WorkflowTool
    ...
    def _init_harness(self, work_dir: str, provider: ProviderConfig) -> None:
        """初始化 Harness 增强组件。"""
        from ecomdevagent.context.critic import CompletenessCritic
        from ecomdevagent.permissions.audit import AuditLogger
        from ecomdevagent.permissions.rate_limit import RateLimiter
        from ecomdevagent.agents.metrics import MetricsCollector
        ...
        from ecomdevagent.harness.tools import (
    ...
    def _init_evolution(self, work_dir: str) -> None:
        ...
        harness_dir = Path(work_dir) / "ecomdevagent" / "harness"
    ...
    def _make_evolution_client(self):
        ...
        from ecomdevagent.tools.base import TextDelta, StreamEnd
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from ecomdevagent.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from ecomdevagent.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from ecomdevagent.plan_dialog import InlinePlanWidget, PlanChoice
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from ecomdevagent.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from ecomdevagent.permission_dialog import InlinePermissionWidget
